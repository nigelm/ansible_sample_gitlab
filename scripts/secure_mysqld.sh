...
### Modified version of mysql_secure_installation
### takes new root password as first argument and
### applies everything, no interaction.

new_root_password=$1
my_temp_dir=`mktemp --directory --tmdir`
cd ${my_temp_dir} || exit 6
...
get_root_password() {
    ...
    password="" ### force initial password unset
    ### stty -echo
...
set_root_password() {
    ...
    ### return 1
    ### fi
    password1="${new_root_password}" ### force in new password
...
cleanup() {
    ...
    rm -f $config $command
    cd /                    ### extra cleanup
    rm -rf ${my_temp_dir}   ### extra cleanup
}
